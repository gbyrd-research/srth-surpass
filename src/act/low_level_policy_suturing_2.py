        ...
        with torch.inference_mode():
            ...
            while t < self.num_inferences:
                try:
                    ...
                    # Explicit synchronization after data transfer to GPU (prevents DataParallel issues)
                    torch.cuda.synchronize()
                    
                    # Run inference (already in torch.inference_mode() context from outer loop)
                    if self.args.policy_class == "SRT" and self.args.use_language:
                        ...
                        # ACT, Diffusion, CNNMLP policies require qpos
                        qpos_zero = torch.zeros(1, 20).float().cuda()
                        action = self.policy(qpos_zero, curr_image, command_embedding=command_embedding).cpu().numpy().squeeze()
                    
                    # Explicit synchronization after forward pass
                    torch.cuda.synchronize()
