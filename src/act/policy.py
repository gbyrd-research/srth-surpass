...
class SRTPolicy(nn.Module):
    def __init__(self, args_override):
        super().__init__()
        args_override['model_type'] = "SRT"
    ...
    def __call__(self, image, actions=None, is_pad=None, command_embedding=None):
        ...
        if actions is not None: # training time
            ...
            loss_dict = dict()
            a_hat = self.model(image, command_embedding=command_embedding)
            ...
            a_hat = self.model(image, command_embedding=command_embedding) # no action, sample from prior
            return a_hat
...
class DiffusionPolicy(nn.Module):
    def __init__(self, args_override):
        ...
        self.num_kp = 32
        if backbone_name == "resnet18film":
            print("using resnet18film backbone")
        ...
        if "resnet18" in backbone_name:
            ...
            self.backbones = replace_bn_with_gn(self.backbones, 8)  
...
class DiffusionPolicyNoSpatialSoftmax(nn.Module):
    def __init__(self, args_override):
        from robomimic.models.base_nets import ResNet18Conv, SpatialSoftmax
        from robomimic.algo.diffusion_policy import (
            replace_bn_with_gn,
        ...
        if backbone_name == "resnet18film":
            ...
            self.feature_dimension = 512
        self.obs_dim = (
            self.feature_dimension * len(self.camera_names))  # camera features and proprio
        if use_language:
            self.obs_dim += self.feature_dimension # TODO: consider deleting
            self.lang_embed_proj = nn.Linear(
                768, self.feature_dimension
            )  # TODO: might change
            if backbone_name == "resnet18film":
        ...
        for _ in self.camera_names:
            if use_language:
                print("Using FiLMed backbone.")
                backbone = FilMedBackbone(backbone_name, average_pool=True)
            else:
                print("Using ResNet18Conv backbone.")
                backbone = ResNet18Conv(
                    **{
        ...
        if "resnet18" in backbone_name:
            ...
            self.backbones = replace_bn_with_gn(self.backbones, 8) # TODO: 8 was added to fix things
    ...
    def __call__(self, qpos, image, actions=None, is_pad=None, command_embedding=None):
        ...
        if actions is not None:  # training time
            ...
            for cam_id in range(len(self.camera_names)):
                ...
                cam_features = torch.flatten(cam_features, start_dim=1)
                out_features = nets["policy"]["linears"][cam_id](cam_features)
                all_features.append(out_features)
            ...
            if command_embedding is None:
                ...
                obs_cond = torch.cat(
                    all_features + [command_embedding_proj], dim=1
                )
