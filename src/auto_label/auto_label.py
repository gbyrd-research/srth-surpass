...
def get_auto_label(ee_csv, start_ts, chunk_size = 10, jaw_threshold=0.3):
...
def get_all_auto_labels_list():
    directions = ["to the left", "to the right", "higher", "lower", "away from me", "towards me"]
    
    left_arm_movements = [f"move left arm {direction}" for direction in directions]
    right_arm_movements = [f"move right arm {direction}" for direction in directions]
    ...
    # Add arm movements
    all_possible_labels.extend(left_arm_movements)
    all_possible_labels.extend(right_arm_movements)
    
    # Add gripper actions
    all_possible_labels.extend(gripper_actions)
    
    # If no future action is detected - for None return value
    all_possible_labels.append("do not move")
