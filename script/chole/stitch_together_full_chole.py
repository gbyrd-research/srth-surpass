...
def create_combined_video(base_path, output_path, num_videos=10, num_phases=17, tissue_idx=1, after_phase_offset = 5, before_phase_offset = 5, phase_text_flag = True, desired_camera_names=["endo_psm2", "left_img_dir", "endo_psm1"], save_each_n_frame_as_image_wo_text=None):
   ...
   # Create the parent directory if it does not exist
    ...
    for vid_index in range(num_videos):
        ...
        for phase_idx in range(1, num_phases + 1):
            ...
            phase_folders = [folder for folder in os.listdir(tissue_folder_path) if folder.startswith(phase_folder_start) and "recovery" not in folder]
            if not phase_folders:
                raise ValueError(f"No folder found for phase index {phase_idx}")
            # Select the first matching phase folder (or adjust the logic as needed)
            phase_folder_path = Path(os.path.join(tissue_folder_path, phase_folders[0]))
            ...
            if not left_img_dir_path.exists():
                ...
                continue
            start, end, _ = get_valid_demo_start_end_indices(selected_date_folder_path, before_phase_offset, after_phase_offset)
...
if __name__ == "__main__":
    ...
    num_videos = 2
    num_phases = 17
    tissue = 47  # tissue_1 has no continuous phases, so we are using >= tissue_4
    before_phase_offset, after_phase_offset = 0, 0
    phase_text_flag = True
    ...
    save_each_n_frame_as_image_wo_text = None
