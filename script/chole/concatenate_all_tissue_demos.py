...
def create_combined_video_all_demos(base_path, output_path, tissue_idx, timestamp, after_phase_offset = 5, before_phase_offset = 5,
                                    with_label_flag=True, desired_camera_names=["endo_psm2", "left_img_dir", "endo_psm1"], tissue_prefix="tissue",
    ...
    for phase_folder_path in phase_folders_sorted:
        ...
        for selected_date_folder_path in date_folder_sorted:
            ...
            if not left_img_dir_path.exists():
                ...
                continue
            start, end, _ = get_valid_demo_start_end_indices(selected_date_folder_path, before_phase_offset, after_phase_offset, use_kinematic_indices_flag)
            ...
            for frame_idx in range(start, end + 1):
                ...
                if with_label_flag:
                    ...
                    font_scale = 2 / abs(5 - len(desired_camera_names)) 
                    cv2.putText(final_image, text, (text_position_x, 30), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 255), 2, cv2.LINE_AA)
...
if __name__ == "__main__":
    ...
    tissue_indices = [18] # [1,4,5,6,8,12,13] # [14,18,19,22,23,30,32,35,39,40,41,47,49]
    tissue_prefix = "tissue" # "tissue" "phantom"
    dataset_name = "experiments" # "base_chole_clipping_cutting" "phantom_chole" "debugging" "debugging2"
    ...
    before_phase_offset, after_phase_offset = 0, 0
    with_label_flag = True
    desired_camera_names = ["left_img_dir"] # None 
    use_kinematic_indices_flag = False
    for tissue_idx in tissue_indices:
        create_combined_video_all_demos(base_path, output_path, tissue_idx, timestamp, after_phase_offset, before_phase_offset, with_label_flag, 
                                        desired_camera_names, tissue_prefix, use_kinematic_indices_flag)
